...
function enableDragAndDrop(li) {
  ...
  li.addEventListener("dragend", () => {
    ...
    // update status based on column it was dropped into
    const parentId = li.parentElement.id.replace("-list", "");
    const allTasks = JSON.parse(localStorage.getItem("tasks")) || [];

    allTasks.forEach(t => {
      if (t.text === li.querySelector("span").textContent && t.category === "work") {
        t.status = parentId;
      }
    });
...
function saveTasks() {
  ...
  // merge with other categories instead of wiping them
  const allTasks = JSON.parse(localStorage.getItem("tasks")) || [];
  const otherTasks = allTasks.filter(t => t.category !== "work");
